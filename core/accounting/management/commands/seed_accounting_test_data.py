...
from accounting.models import ChartOfAccount, JournalEntry, AccountingIntegration, FiscalYear, FiscalPeriod
from billing.models import Plan, Subscription, PlanTier
from invoicing.models import Invoice, Payment, PaymentType, PaymentProviderConfig, PaymentMethod
    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                # 2.5 Setup Fiscal Year and Periods
                self.stdout.write("Setting up Fiscal Year and Periods...")
                now = timezone.now()
                ...
                for i in range(1, 13):
                    start = now.replace(month=i, day=1).date()
                    # End is last day of month
                    if i == 12:
                        end = now.replace(month=12, day=31).date()
                    else:
                        end = (now.replace(month=i+1, day=1) - timezone.timedelta(days=1)).date()
                ...
                tier, _ = PlanTier.objects.get_or_create(tenant=tenant, tier_name='standard', defaults={'label': 'Standard'})
                plan, _ = Plan.objects.get_or_create(
                    name='Pro Monthly', 
                    defaults={'price': 500, 'max_users': 10, 'is_active': True}
                )
                ...
                payment_type, _ = PaymentType.objects.get_or_create(tenant=tenant, type_name='card', defaults={'label': 'Card'})
                provider, _ = PaymentProviderConfig.objects.get_or_create(tenant=tenant, provider_config_name='stripe', defaults={'display_name': 'Stripe'})
                pm, _ = PaymentMethod.objects.get_or_create(
                    tenant=tenant, user=user, type='card',
                    defaults={'display_info': '**** 4242', 'provider': provider}
                )
                ...
                warehouse, _ = Warehouse.objects.get_or_create(
                    tenant=tenant, warehouse_code='MAIN', 
                    defaults={'warehouse_name': 'Main Store'}
                )
                ...
                pos_trans = POSTransaction.objects.create(
                    tenant=tenant, terminal=terminal, cashier=user,
                ...
                POSPayment.objects.create(
                    ...
                    amount=150, payment_method='cash', status='completed',
                    processed_by=user
                )
