...
# Platform models are now in core.billing
from core.billing.models import PaymentProvider, PaymentProviderConfig, Payment, Invoice
        ...
        return [
            {
                ...
                "supported_currencies": ["USD", "EUR"], # Placeholder
            }
            for config in configs
        ]
    else:
        # Get all active platform providers
        providers = PaymentProviderConfig.objects.filter(config_is_active=True)
        return [
            {
                "name": p.provider_config_name,
