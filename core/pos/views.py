...
class SessionOpenView(LoginRequiredMixin, TenantAwareViewMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Check for existing active sessions for THIS user on ANY terminal
        user_sessions = POSSession.objects.filter(
            cashier=self.request.user, status='active'
        ).select_related('terminal')
        
        context['user_active_sessions'] = list(user_sessions)
        
        # If user has exactly one active session, provide shortcut to continue
        if user_sessions.count() == 1:
            context['existing_session'] = user_sessions.first()
...
class ServiceWorkerView(View):
    """View to serve the Service Worker JS file with correct MIME type."""
    def get(self, request):
        # We can either read the file from static or define logic here
        # For simplicity, we'll serve it from a separate file in static
        # but through this view to ensure correct scope and MIME type
        from django.conf import settings
        import os
        
        sw_path = os.path.join(settings.BASE_DIR, 'core', 'static', 'pos', 'js', 'sw.js')
        try:
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            for pm in payment_methods:
                ...
                ref = pm.get('reference_number', '')
                redeem = pm.get('redeem_points', False)
                
                payment = POSService.add_payment(
                    transaction=transaction,
            ...
            return JsonResponse({
                ...
                'total_amount': str(transaction.total_amount),
                'paid_amount': str(transaction.amount_paid),
                'remaining': str(transaction.total_amount - transaction.amount_paid),
...
class OfflineTransactionSyncAPI(LoginRequiredMixin, View):
    """API for syncing transactions created while offline."""
    ...
    def post(self, request):
        ...
        try:
            ...
            for pm in data.get('payments', []):
                POSService.add_payment(
                    transaction=transaction,
                    payment_method=pm['method'],
                    amount=pm['amount'],
                    reference_number=pm.get('reference_number', ''),
                    cashier=request.user
                )
...
class PrinterDiscoveryAPI(LoginRequiredMixin, View):
    """API for discovering network printers."""
    
    def get(self, request):
        subnet = request.GET.get('subnet')
        try:
            printers = ReceiptPrinter.discover_network_printers(subnet)
...
class DashboardStatsAPI(LoginRequiredMixin, View):
    """API for real-time dashboard statistics."""
    ...
    def get(self, request):
        ...
        sales_total = txns.aggregate(Sum('total_amount'))['total_amount__sum'] or 0
        txn_count = txns.count()
        
        # Hourly Sales (Peak Hours)
        hourly_sales = txns.annotate(
        ...
        category_sales = POSTransactionLine.objects.filter(
            transaction__tenant=tenant,
            transaction__status='completed',
            transaction__completed_at__date=today
        ...
        return JsonResponse({
            ...
            'today_transactions': txn_count,
            'hourly_sales': [
                {'hour': h['hour'].strftime('%H:00'), 'total': float(h['total'])} 
                for h in hourly_sales
            ],
            'category_sales': [
                {'category': c['product__category__name'] or 'Uncategorized', 'total': float(c['total'])}
...
class TransactionDetailAPI(LoginRequiredMixin, View):
    ...
    def patch(self, request, pk):
        """Update transaction fields (e.g. notes, discount)."""
        transaction = get_object_or_404(
            POSTransaction, pk=pk, tenant=request.user.tenant
        )
        data = json.loads(request.body)
        ...
        if 'discount_percent' in data or 'discount_amount' in data:
            discount_percent = data.get('discount_percent', 0)
            discount_amount = data.get('discount_amount', 0)
            try:
                POSService.apply_discount(
                    transaction, 
        ...
        return JsonResponse({
            'success': True,
            'notes': transaction.notes,
            'discount_percent': str(transaction.discount_percent),
            'discount_amount': str(transaction.discount_amount),
            'total': str(transaction.total_amount)
        })
    ...
    def post(self, request, pk):
        ...
        try:
            if action == 'apply_coupon':
                ...
                POSService.apply_coupon(transaction, coupon_code)
                return JsonResponse({
                    'success': True,
                    'discount_amount': str(transaction.discount_amount),
                    'total': str(transaction.total_amount)
                ...
                POSService.remove_coupon(transaction)
                return JsonResponse({
                    'success': True,
                    'discount_amount': str(transaction.discount_amount),
                    'total': str(transaction.total_amount)
