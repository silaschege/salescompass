...
from invoicing.services import TaxService
...
class POSService:
    ...
    def add_line_item(transaction, product, quantity=1, unit_price=None, 
        ...
        If the same product already exists in the transaction, increment its quantity.
        """
        if transaction.status != 'draft':
            raise ValueError("Cannot modify a non-draft transaction.")
        
        # Check for existing line with same product
        existing_line = POSTransactionLine.objects.filter(
        ...
        if existing_line:
            ...
            existing_line.quantity += Decimal(str(quantity))
            existing_line.save()
    ...
    @staticmethod
    @transaction.atomic
    def add_payment(transaction, payment_method, amount, cashier, 
        ...
        if total_paid >= transaction.total_amount:
            ...
            from invoicing.services import AccountingIntegrationService
            try:
        ...
        if payment_method == 'cash':
            POSService._record_cash_movement(
                transaction, 'sale', amount, cashier
            )
    ...
    @staticmethod
    def _record_cash_movement(transaction, movement_type, amount, cashier):
        """
