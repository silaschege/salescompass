...
def calculate_commission(opportunity):
    ...
    return None
def calculate_preview_commission(user, amount, product=None, plan=None):
    """
    Simultate commission calculation for 'what-if' scenarios.
    Does not require a saved Opportunity object.
    """
    if not user:
        return None

    # Use specified plan or find active plan for now
    if not plan:
        active_plan_assignment = UserCommissionPlan.objects.filter(
            tenant=user.tenant,
            user=user,
    ...
    rules = plan.rules.all()
    basis_amount = Decimal(str(amount))
    
    # Handle basis types
    if plan.basis == 'margin':
        basis_amount = basis_amount * Decimal('0.20') # Default 20% margin for preview
    
    # In a real what-if, we might not have 'current performance' unless we pass it
    # We'll fetch current period performance for accelerators/decelerators
    current_performance_amount = Decimal('0.00')
    if any(r.rate_type in ['accelerator', 'decelerator'] for r in rules):
        ...
        current_performance_amount = Opportunity.objects.filter(
            ...
            owner=user,
            stage__is_won=True,
            close_date__range=[start_of_month, today]
        ).aggregate(total=Sum('amount'))['total'] or Decimal('0.00')
    ...
    for rule in rules:
        # Product check
        if rule.product and product and rule.product != product:
            continue
    ...
    if applied_rule:
        total_commission = basis_amount * (applied_rate / 100)
        
        # Handle split
        if applied_rule.split_with:
            split_pct = applied_rule.split_percentage or Decimal('0.00')
            split_amount = total_commission * (split_pct / 100)
            return total_commission - split_amount, applied_rate, applied_rule
            
        return total_commission, applied_rate, applied_rule
