...
from .utils import get_user_performance, calculate_forecast, get_earnings_trend, calculate_pace, calculate_preview_commission
...
class CommissionPlanUpdateView(SalesCompassUpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['rule_form'] = CommissionRuleForm(plan=self.object, tenant=self.request.user.tenant)
        
        # Add products and users for the rule form
        from products.models import Product
        from django.contrib.auth import get_user_model
        User = get_user_model()
        context['products'] = Product.objects.filter(tenant=self.request.user.tenant)
        context['users'] = User.objects.filter(tenant=self.request.user.tenant, is_active=True)
...
class CommissionStatementView(SalesCompassListView):
    model = Commission
    template_name = 'commissions/statement_web.html'
    context_object_name = 'commissions'
    paginate_by = 20
...
class WhatIfCalculationAPIView(LoginRequiredMixin, View):
    """
    API endpoint for What-If Calculator to get accurate commission projections.
    """
    def post(self, request, *args, **kwargs):
        try:
            ...
            plan = CommissionPlan.objects.filter(id=plan_id).first() if plan_id else None
            
            result = calculate_preview_commission(
                user=request.user,
                amount=amount,
                product=product,
                plan=plan
            ...
            if result:
                commission_amount, applied_rate, applied_rule = result
                return JsonResponse({
                    'success': True,
                    'commission_amount': float(commission_amount),
                    'applied_rate': float(applied_rate),
                    'applied_rule': str(applied_rule) if applied_rule else 'Default'
                })
            
            return JsonResponse({'success': False, 'error': 'Calculation failed'}, status=400)
