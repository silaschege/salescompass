...
class CommissionRuleForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Filter products and users by tenant if provided
        if tenant:
            self.fields['product'].queryset = Product.objects.filter(tenant=tenant)
            self.fields['split_with'].queryset = User.objects.filter(tenant=tenant, is_active=True)
        else:
            self.fields['product'].queryset = Product.objects.none()
            self.fields['split_with'].queryset = User.objects.none()
