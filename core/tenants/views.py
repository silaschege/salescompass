...
class OnboardingWizardBaseView(LoginRequiredMixin, TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        # Superusers (control plane) can always access the onboarding wizard to set up new tenants
        if request.user.is_authenticated and request.user.is_superuser:
            return super().dispatch(request, *args, **kwargs)
        # Regular users with an existing tenant should be redirected to their dashboard
...
class TenantListView(SalesCompassListView):
    ...
    def get_queryset(self):
        ...
        return queryset.select_related('tenant_admin', 'plan')
    ...
    def get_context_data(self, **kwargs):
        ...
        context['stats'] = {
            ...
            'active': all_tenants.filter(subscription_status='active').count(),
            'suspended': all_tenants.filter(subscription_status='suspended').count(),
            'trial': all_tenants.filter(subscription_status='trialing').count(),
        }
        return context
