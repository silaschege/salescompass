...
from .models import (
    ...
    StockAdjustment, ReorderRule, StockAlert, WarehouseZone, SerialNumber
)
from .forms import (
    WarehouseForm, StockAddForm, StockRemoveForm, StockTransferForm,
    StockAdjustmentForm, ReorderRuleForm,
    TransferCreateForm, TransferLineFormSet, WarehouseZoneForm
)
...
class WarehouseZoneListView(SalesCompassListView):
    """List all warehouse zones."""
    model = WarehouseZone
    template_name = 'inventory/zone_list.html'
    context_object_name = 'zones'
    
    def get_queryset(self):
        return WarehouseZone.objects.filter(tenant=self.request.user.tenant)


class WarehouseZoneCreateView(SalesCompassCreateView):
    """Create a new warehouse zone."""
    model = WarehouseZone
    form_class = WarehouseZoneForm
    template_name = 'inventory/zone_form.html'
    success_url = reverse_lazy('inventory:zone_list')


class WarehouseZoneUpdateView(SalesCompassUpdateView):
    """Edit an existing warehouse zone."""
    model = WarehouseZone
    form_class = WarehouseZoneForm
    template_name = 'inventory/zone_form.html'
    success_url = reverse_lazy('inventory:zone_list')
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            try:
                ...
                serial_numbers_raw = form.cleaned_data.get('serial_numbers', '')
                serial_numbers = []
                if serial_numbers_raw:
                    # Split by comma or newline and strip whitespace
                    import re
                    serial_numbers = [sn.strip() for sn in re.split(r'[,\n\r]+', serial_numbers_raw) if sn.strip()]
...
class TransferDetailView(SalesCompassDetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['lines'] = self.object.lines.select_related('product', 'from_location', 'to_location')
        return context


class TransferShipView(LoginRequiredMixin, TenantAwareViewMixin, TemplateView):
    """View for shipping a transfer, allowing location and SN selection."""
    template_name = 'inventory/transfer_ship.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Filter locations to only show those in the source warehouse
        from .models import StockLocation
        context['locations'] = StockLocation.objects.filter(
    ...
    def post(self, request, pk):
        ...
        for key, value in request.POST.items():
            if key.startswith('location_'):
                ...
                if line_id not in shipment_data: shipment_data[line_id] = {}
                shipment_data[line_id]['from_location'] = value
            elif key.startswith('sns_'):
                line_id = key.split('_')[1]
                if line_id not in shipment_data: shipment_data[line_id] = {}
                shipment_data[line_id]['serial_numbers'] = value

        try:
            TransferService.ship_transfer(transfer, request.user, shipment_data)
...
class TransferReceiveView(LoginRequiredMixin, TenantAwareViewMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['transfer'] = transfer
        context['lines'] = transfer.lines.select_related('product')
        # Filter locations to only show those in the destination warehouse
        from .models import StockLocation
        context['locations'] = StockLocation.objects.filter(
    ...
    def post(self, request, pk):
        ...
        for key, value in request.POST.items():
            if key.startswith('qty_'):
                line_id = key.split('_')[1]
                if line_id not in receipt_data: receipt_data[line_id] = {}
                receipt_data[line_id]['quantity'] = value
            elif key.startswith('location_'):
                line_id = key.split('_')[1]
                if line_id not in receipt_data: receipt_data[line_id] = {}
                receipt_data[line_id]['to_location'] = value
            elif key.startswith('sns_'):
                line_id = key.split('_')[1]
                if line_id not in receipt_data: receipt_data[line_id] = {}
                receipt_data[line_id]['serial_numbers'] = value
                
        try:
            TransferService.receive_transfer(transfer, receipt_data, request.user)
...
# Valuation Reports
class StockValuationReportView(LoginRequiredMixin, TenantAwareViewMixin, TemplateView):
    """View for inventory valuation report."""
    template_name = 'inventory/valuation_report.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        for stock in stock_levels:
            w_id = stock.warehouse.id
            if w_id not in warehouse_data:
                warehouse_data[w_id] = {
            ...
            value = stock.quantity * (stock.cost_price or Decimal('0'))
            warehouse_data[w_id]['total_items'] += 1
            warehouse_data[w_id]['total_value'] += value
            warehouse_data[w_id]['items'].append({
        ...
        context['warehouse_data'] = warehouse_data.values()
        context['total_inventory_value'] = total_value
        return context


class StockValuationExportView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        stock_levels = StockLevel.objects.filter(
            tenant=tenant,
            quantity__gt=0
        ).select_related('product', 'warehouse').order_by('warehouse__warehouse_name', 'product__product_name')
        
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="inventory_valuation_{timezone.now().strftime("%Y%m%d")}.csv"'
        
        writer = csv.writer(response)
        writer.writerow(['Warehouse', 'Product SKU', 'Product Name', 'Quantity', 'Cost Price', 'Total Value'])
...
class ProductBarcodeLookupView(LoginRequiredMixin, TenantAwareViewMixin, View):
    """Lookup product details by barcode (SKU or UPC)."""
    ...
    def get(self, request, *args, **kwargs):
        ...
        tenant = request.user.tenant
        product = Product.objects.filter(tenant=tenant).filter(
            Q(sku=barcode) | Q(upc=barcode)
        ).first()
