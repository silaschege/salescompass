...
class WarehouseZone(TenantModel):
    """
    Specific zones or areas within a warehouse (e.g. Cold Storage, Dry Goods, Bulk Picking).
    """
...
class StockLevel(TenantModel):
    ...
    acquisition_date = models.DateField(auto_now_add=True, null=True, blank=True)
    expiry_date = models.DateField(null=True, blank=True, help_text="Expiry date for FEFO")
    last_valuation_date = models.DateField(null=True, blank=True)

    class Meta:
        ordering = ['product', 'warehouse', 'acquisition_date']
...
class InterStoreTransferLine(TenantModel):
    ...
    # Serial numbers tracking (for items in transit)
    serial_numbers = models.TextField(blank=True, help_text="Comma-separated serial numbers being transferred")
...
class SerialNumber(TenantModel):
    """
    Unique serial number for an individual item of a product.
    """
    STATUS_CHOICES = [
        ('in_stock', 'In Stock'),
        ('allocated', 'Allocated'),
