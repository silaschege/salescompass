...
from .models import (
    ...
    InterStoreTransfer, WarehouseZone
)
...
class WarehouseZoneForm(forms.ModelForm):
    """Form for creating/editing warehouse zones."""
    
    class Meta:
        model = WarehouseZone
        fields = ['warehouse', 'zone_code', 'zone_name', 'description', 'is_active']
        widgets = {
...
class StockLocationForm(forms.ModelForm):
    ...
    class Meta:
        model = StockLocation
        fields = [
            'warehouse', 'zone', 'location_code', 'location_name',
    ...
    def __init__(self, *args, tenant=None, **kwargs):
        ...
        if tenant:
            self.fields['warehouse'].queryset = Warehouse.objects.filter(tenant=tenant, is_active=True)
            self.fields['zone'].queryset = WarehouseZone.objects.filter(tenant=tenant, is_active=True)
...
class StockAddForm(forms.Form):
    ...
    batch_number = forms.CharField(max_length=100, required=False, help_text="Batch/Lot number")
    expiry_date = forms.DateField(required=False, widget=forms.DateInput(attrs={'type': 'date'}))
    serial_numbers = forms.CharField(
        widget=forms.Textarea(attrs={'rows': 3, 'placeholder': 'Enter serial numbers separated by commas or newlines'}), 
        required=False,
        help_text="Required for products with serial tracking"
    )
