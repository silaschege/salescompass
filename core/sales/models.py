...
class Sale(TenantModel, TimeStampedModel):
    ...
    def generate_invoice(self):
        """
        Create a tenant invoice for this sale.
        """
        ...
        invoice = Invoice.objects.create(
            ...
            invoice_number=f"INV-S-{self.id}-{timezone.now().strftime('%m%d')}"
        )
        ...
        InvoiceLine.objects.create(
            ...
            unit_price=self.amount / self.quantity if self.quantity > 0 else self.amount,
            description=f"Sale of {self.product.product_name}"
        )
