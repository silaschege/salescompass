...
# Dynamic path handling to resolve imports regardless of execution context
try:
    ...
    # If absolute import fails, we might be running from a different context
    # Try adding project root to sys.path
    import sys
    import os
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    # Go up 4 levels to reach project root (containing ml_models package)
    ...
    try:
        ...
        # If that still fails, maybe we are inside ml_models and need to import infrastructure directly
        # Go up 3 levels to reach ml_models directory
        ml_models_dir = os.path.abspath(os.path.join(current_dir, "../../.."))
        if ml_models_dir not in sys.path:
            sys.path.insert(0, ml_models_dir)
            
        try:
            from infrastructure.config.ontology_config import ModelSpecification, ModelType
            from infrastructure.config.settings import config
        except ImportError as e:
            # If all fails, re-raise the error
            raise ImportError(f"Could not import infrastructure config. Checked paths: {project_root}, {ml_models_dir}. Error: {e}")
